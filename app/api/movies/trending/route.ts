...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ movies, totalResults: response.total_results, timeWindow });
    } catch (error) {
        console.error('Trending movies API error:', error);
        return NextResponse.json(
            { error: 'Failed to get trending movies', details: error instanceof Error ? error.message : 'Unknown error' },
            { status: 500 }
        );
    }
}
